...
export async function generate_ctx_I_file(opts = {}) {
    const project_path = (opts === null || opts === void 0 ? void 0 : opts.project_path) || '.';
    const src_relative_path = (opts === null || opts === void 0 ? void 0 : opts.src_relative_path) || 'src';
    const exclude = (opts === null || opts === void 0 ? void 0 : opts.exclude) || [];
    const b_h_b = (opts === null || opts === void 0 ? void 0 : opts.b_h_b) || false;
    ...
    const unfiltered_b_name_a = unfiltered_b_path_a.map(b_path => sanitize(basename(b_path, '.ts')));
    const unfiltered_base_name_a = unfiltered_b_name_a.map(b_name => sanitize(strip_to_base_name(b_name)));
    const exclude_idx_a = unfiltered_base_name_a.reduce((memo, base_name, idx) => {
        if (~exclude.indexOf(base_name)) {
            memo.push(idx);
        }
        return memo;
    }, []);
    const filter_fn = (_, idx) => !~exclude_idx_a.indexOf(idx);
    ...
    const T_name_a = base_name_a.map(base_name => `${base_name}_T`);
...
//# sourceMappingURL=src/generate_ctx_I_file.js.map